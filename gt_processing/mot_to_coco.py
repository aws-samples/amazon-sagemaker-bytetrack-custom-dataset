...
MOT_PATH = '/opt/ml/processing/mot'
DATA_PATH = '/opt/ml/processing/coco'
suffix = 'jpeg'

OUT_PATH = os.path.join(DATA_PATH, 'annotations')
SPLITS = ['val', 'train', 'test']  # --> split training data to train_half and val_half.
HALF_VIDEO = False
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            num_images = len([image for image in images if 'jpeg' in image])  # half and half
            ...
            print(f'image_range: {image_range}')
            for i in range(num_images):
                if i < image_range[0] or i > image_range[1]:
                    continue
                #img = cv2.imread(os.path.join(data_path, '{}/img1/frame_{:04d}.{}'.format(seq, i, suffix)))
                #height, width = img.shape[:2]
                im = Image.open(os.path.join(data_path, '{}/img1/frame_{:04d}.{}'.format(seq, i, suffix)))
                width, height = im.size
                image_info = {'file_name': '{}/img1/frame_{:04d}.{}'.format(seq, i, suffix),  # image name.
